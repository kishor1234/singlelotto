...
class calculateResult extends CAaskController {
    ...
    public function create() {
        ...
        $result1 = $this->adminDB[$_SESSION["db_1"]]->query($sql);
        $row1 = $result1->fetch_assoc();
        $ra = rand(0, 1);
        if ($ra == 0) {
            $this->prev = json_decode($row1["prev"], true);
        }
                                    ...
                                    if (!in_array($nono, $this->blockno)) {
                                        ...
                                        if (!in_array($nono, $lottery)) {
                                            ...
                                            if (strcmp($this->min, "1") == 0) {
                                                //if ($this->checkPrevious($this->load[$key][$nono])) {
                                                    if ($this->load[$key][$nono] <= $cpoint && $this->load[$key][$nono] > $this->min) {
                                                        $perPl = $pointPerPlat;
                                                        $cpoint -= $this->load[$key][$nono];
                                                        if (!$this->set) {
                                                            $this->set = true;
                                                            array_push($this->newprev, $this->load[$key][$nono]);
                                                        }
...
//                        print_r($lottery);
            ...
            $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->update(array("prev" => json_encode($this->newprev)), "admin") . $this->ask_mysqli->whereSingle(array("id" => 1)));
...
//        }
    ...
    function checkPrevious($num) {
        if (!empty($this->prev)) {
            if (in_array($num, $this->prev)) {
                return false;
            }
        }
        return true;
