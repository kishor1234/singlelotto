...
class selldistributorreportsall extends CAaskController
{
    ...
    public function initialize()
    {
        ...
        try {
            ...
            $sl = $this->ask_mysqli->select("subadmin", $_SESSION["db_1"]); // . $this->ask_mysqli->whereBetweenDates('on_create', $request["dateform"], $request["dateto"]) . " AND active='1'";
            // print_r($request);
            $sl .= $this->ask_mysqli->whereSingle(array("m" => $request["m"]));
            // if (isset($request["m"])) {
            //     switch ($request["m"]) {
            //         case "main":
            //             $sl .= $this->ask_mysqli->whereSingle(array("created_by" => $request["id"]));
            //             break;
            //         default;
            //             break;
            //     }
            // } else {
            // }
            // $sl = "SELECT `agent`.`createBy` as userid FROM `agent` INNER JOIN subadmin ON `agent`.`createBy`= `subadmin`.`userid` WHERE `subadmin`.created_by='{$request["id"]}' GROUP BY `agent`.`createBy` ORDER BY `agent`.`createBy` ASC";
            // echo json_encode(array("s" => $sl));
            // die;
