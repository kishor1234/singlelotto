...
class CAgentChangePassword extends CAaskController {
    ...
    function changePassword($postData) {
        $data = null;
        try {
            $sql = $this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("mobileno" => $this->encript->decTxt($postData["etoken"]), "userid" => $this->encript->decTxt($postData["token"])), "AND");
            $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
            if ($row = $result->fetch_assoc()) {
                if (strcmp($row["mobileno"], $this->encript->decTxt($postData["etoken"])) == 0) {
                    $sql = $this->ask_mysqli->update(array("password" => password_hash($postData["pass"], PASSWORD_DEFAULT)), "agent") . $this->ask_mysqli->where(array("mobileno" => $this->encript->decTxt($postData["etoken"]), "userid" => $this->encript->decTxt($postData["token"])), "AND");
