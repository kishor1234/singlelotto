...
class agentlogin extends CAaskController {
    ...
    function checkLoginUser($postData) {
        $data = null;
        try {
            $sql = $this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("userid" => $postData["userName"], "is_active" => 1), "AND");
            $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
            if ($row = $result->fetch_assoc()) {
                if (password_verify($postData["password"], $row["password"])) {
                    $data = array("message" => "success", "_id" => $row["agent_id"], "user_id" => $row["userid"], "mobile" => $row["mobileno"], "ap" => $row["winper"], "commission" => $row["comission"],"createBy" => $row["createBy"]);
                } else {
                    $data = null;
                }
            } else {
