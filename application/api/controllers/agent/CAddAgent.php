...
class CAddAgent extends CAaskController {
    ...
    function loadGame() {
        try {
            ...
            while ($row = $query->fetch_assoc()) {
                ...
                $subdata[] = '<button onclick="deleteGame(' . $row["agent_id"] . ',0)" class="btn btn-danger btn-xs"> <i class="fa fa-trash-o"></i></button>';
                $data[] = $subdata;
            }
            $json_data = array(
    ...
    function deleteGame() {
        $sql = $this->ask_mysqli->select("hostgame", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("agent_id" => $_POST["agent_id"]));
        $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
        if ($row = $result->fetch_assoc()) {
            unlink($row["path"]);
            $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->delete("hostgame") . $this->ask_mysqli->whereSingle(array("agent_id" => $_POST["agent_id"])));
    ...
    function blockno() {
        ...
        $sql = $this->ask_mysqli->update(array("blockno" => $blockno), "admin") . $this->ask_mysqli->whereSingle(array("agent_id" => $id));
        if ($this->adminDB[$_SESSION["db_1"]]->query($sql)) {
    ...
    function getBlockno() {
        $resutl = $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->select("admin", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("agent_id" => $_POST["agent_id"])));
    ...
    function loadTable() {
        try {
            ...
            while ($row = $query->fetch_assoc()) {
                ...
                $subdata[] = $row['isdate'];
                $active = '<button onclick="deleteFullAccount(\'' . $row["agent_id"] . '\',0)" class="btn btn-danger btn-xs"> <i class="fa fa-trash-o"></i></button>';
                $active .= ' <button onclick="editAccount(\'' . $row["agent_id"] . '\',\'' . $row["name"] . '\',\'' . $row["mobileno"] . '\',\'' . $row["comission"] . '\',\'' . $row["device"] . '\',\'' . $row["winper"] . '\',\'' . $row["auto"] . '\')" data-toggle="modal" data-target="#myModal" class="btn btn-warning btn-xs"> <i class="fa fa-edit"></i> Edit</button>';
                ...
                $ac = array(0 => 1, 1 => 0);
                $subdata[] = $active . ' <button onclick="suspendAccount(' . $row["agent_id"] . ',' . $ac[$row["is_active"]] . ')" data-toggle="modal" data-target="#myModal" class="btn btn-' . $b[$row['is_active']] . ' btn-xs"> <i class="fa fa-scissors"></i>' . $b2[$row['is_active']] . '</button>';
...
//                $subdata[] = $row['isdate'];
                $active = '<button onclick="deleteFullAccount(\'' . $row["agent_id"] . '\',0)" class="btn btn-danger btn-xs"> <i class="fa fa-trash-o"></i></button>';
                $active .= ' <button onclick="editAccount(\'' . $row["agent_id"] . '\',\'' . $row["name"] . '\',\'' . $row["mobileno"] . '\',\'' . $row["comission"] . '\',\'' . $row["device"] . '\',\'' . $row["winper"] . '\',\'' . $row["auto"] . '\')" data-toggle="modal" data-target="#myModal" class="btn btn-warning btn-xs" title="Edit"> <i class="fa fa-edit"></i> </button>';
                ...
                $b2 = array(1 => "Suspend", 0 => "Active");
                $ac = array(0 => 1, 1 => 0);
                $subdata[] = $active . ' <button onclick="suspendAccount(' . $row["agent_id"] . ',' . $ac[$row["is_active"]] . ')" data-toggle="modal" data-target="#myModal" class="btn btn-' . $b[$row['is_active']] . ' btn-xs"> <i class="fa fa-' . $state . '"></i></button>';
...
//            }
            $userid = "A" . date("Y") . date("m") . date("d") . $last_id;
            $sql = $this->ask_mysqli->update(array("userid" => $userid), "agent") . $this->ask_mysqli->whereSingle(array("agent_id" => $last_id));
            $this->adminDB[$_SESSION["db_1"]]->query($sql) != true ? array_push($error, $this->adminDB[$_SESSION["db_1"]]->error) : true;
            if (empty($error)) {
...
//            }
    ...
    function deleteAgent() {
        ...
        $result = $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $where);
        if ($row = $result->fetch_assoc()) {
            $sdlu = $this->ask_mysqli->select("enduser", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("agent_id" => $row["userid"]));
        ...
        if ($row = $result->fetch_assoc()) {
            ...
            $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->_updateINC(array("balance" => "balance-" . $row["balance"]), "agent") . $this->ask_mysqli->whereSingle(array("userid" => $row["userid"]))) != 1 ? array_push($erroe, "Erro(001) Unable to Update Balance.. agent") : true;
            ...
            $s = $this->ask_mysqli->insert("transaction", array("userid" => $row["userid"], "debit" => $row["balance"], "remark" => "Admin Get Back Balance from Agent and Delete Account", "ip" => $_SERVER["REMOTE_ADDR"], "balance" => $this->getData($this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("userid" => $row["userid"])), "balance")));
            $this->adminDB[$_SESSION["db_1"]]->query($s) != true ? array_push($erroe, "Error on Transcation Table Error Code 02 agent transaction") : true;
            $s1 = $this->ask_mysqli->insert("transaction", array("userid" => $_POST["id"], "credit" => $row["balance"], "remark" => "Self Credit form admin to transfer balance from agent {$row["userid"]} and delete it account", "ip" => $_SERVER["REMOTE_ADDR"], "balance" => $this->getData($this->ask_mysqli->select("admin", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("id" => $_POST["id"])), "balance")));
...
//            $this->adminDB[$_SESSION["db_1"]]->rollback();
    ...
    function update() {
        try {
            ...
            $error = array();
            $this->adminDB[$_SESSION["db_1"]]->autocommit(false);
            $sql = $this->ask_mysqli->update($data, "agent") . $where;
            $this->adminDB[$_SESSION["db_1"]]->query($sql) != true ? array_push($error, $this->adminDB[$_SESSION["db_1"]]->error) : true;
            if (empty($error)) {
                $this->adminDB[$_SESSION["db_1"]]->commit();
                $json_error = json_encode($error);
    ...
    function putPoint() {
        try {
            ...
            if ((float) $row["balance"] > (float) $data["point"]) {
                ...
                $this->adminDB[$_SESSION["db_1"]]->query($sql) != true ? array_push($error, $this->adminDB["db_1"]->error) : true;
                $sql = $this->ask_mysqli->insert("transaction", array("userid" => $data["userid"], "credit" => $data["point"], "remark" => "Admin Send Balance to Agent", "ip" => $_SERVER["REMOTE_ADDR"], "balance" => $this->getData($this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("userid" => $data["userid"])), "balance")));
                $this->adminDB[$_SESSION["db_1"]]->query($sql) != true ? array_push($error, $this->adminDB["db_1"]->error) : true;
    ...
    function getPoint() {
        try {
            ...
            if ((float) $row["balance"] >= (float) $data["point"]) {
                ...
                $sql = $this->ask_mysqli->_updateINC(array("balance" => "balance+" . $data["point"]), "admin") . $this->ask_mysqli->whereSingle(array("id" => $_POST["id"]));
                $this->adminDB[$_SESSION["db_1"]]->query($sql) != true ? array_push($error, $this->adminDB["db_1"]->error) : true;
                $sql = $this->ask_mysqli->insert("transaction", array("userid" => $data["userid"], "debit" => $data["point"], "remark" => "Admin get Back Balance From Agent", "ip" => $_SERVER["REMOTE_ADDR"], "balance" => $this->getData($this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("userid" => $data["userid"])), "balance")));
                $this->adminDB[$_SESSION["db_1"]]->query($sql) != true ? array_push($error, $this->adminDB["db_1"]->error) : true;
    ...
    function suspend() {
        try {
            $data = $_POST;
            unset($data["action"]);
            $where = $this->ask_mysqli->whereSingle(array("agent_id" => $data["id"]));
            unset($data["id"]);
            $this->adminDB[$_SESSION["db_1"]]->autocommit(false);
            $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->update($data, "agent") . $where) != true ? array_push($error, $this->adminDB[$_SESSION["db_1"]]->error) : true;
            if (empty($error)) {
                $this->adminDB[$_SESSION["db_1"]]->commit();
                echo json_encode(array("toast" => array("success", "Agent", "Information Update Success... "), "status" => 1, "message" => "Agent Information Update Success.. "));
            } else {
    ...
    function updateper() {
        try {
            $sql = $this->ask_mysqli->update(array("resultper" => $_POST["resultper"], "min" => $_POST["min"]), "admin") . $this->ask_mysqli->whereSingle(array("agent_id" => $_POST["agent_id"]));
            if ($this->adminDB[$_SESSION["db_1"]]->query($sql)) {
    ...
    function adminBalance() {
        ...
        $data = array();
        $sql = $this->ask_mysqli->select("agent", $_SESSION["db_1"]) . $this->ask_mysqli->whereSingle(array("userid" => $_POST["userid"]));
        $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
        $row = $result->fetch_assoc();
        $data["3"] = $row["balance"];
        ...
        $sql = $this->ask_mysqli->selectCount("enduser", "id") . $this->ask_mysqli->whereSingle(array("agent_id" => $_POST["userid"]));
        $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
        $row = $result->fetch_assoc();
        $data["1"] = $row["count(id)"];
        $data["2"] = 1;
    ...
    function message() {
        unset($_POST["action"]);
        $this->adminDB[$_SESSION["db_1"]]->autocommit(false);
        $this->adminDB[$_SESSION["db_1"]]->query($this->ask_mysqli->update($_POST, "message") . $this->ask_mysqli->whereSingle(array("agent_id" => 1))) != true ? array_push($error, $this->adminDB[$_SESSION["db_1"]]->error) : true;
        if (empty($error)) {
            $this->adminDB[$_SESSION["db_1"]]->commit();
    ...
    function allAgentmain() {
        try {
            $sql = $this->ask_mysqli->select("agent", $_SESSION["db_1"]) . " where createBy!='1'";
            $result = $this->adminDB[$_SESSION["db_1"]]->query($sql);
            $array = array();
            while ($row = $result->fetch_assoc()) {
                unset($row["password"]);
