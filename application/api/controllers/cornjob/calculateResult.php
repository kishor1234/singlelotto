...
//                                print_r($this->idLoadNonZero);
                                ...
                                if (!empty($this->idLoadNonZero[$rowSereis["id"]][$seriesAlphaindex[$keya]])) {
                                    ...
                                    foreach ($this->idLoadNonZero[$rowSereis["id"]][$seriesAlphaindex[$keya]] as $k => $v) {
                                        if ($cpoint >= 0) {
                                            if (!in_array($k, $lottery)) {
                                                print_r($lottery);
                                                echo "{$kay}={$lottery[$k]}=191<br>";
                                                ...
                                                echo $cpoint . "CPT<br>";
                                                $key = 100 * array_search($keya, $seriesAlphaindex);
                                                if ($this->load[$key][$k] != 0) {

                                                    if ($this->load[$key][$k] >= $test) {//&& $this->load[$key][$k] <= $cpoint
                                                        $perPl = $pointPerPlat;
                                                        $cpoint -= $this->load[$key][$k];
                                                        echo $cpoint . " non ZSU [{$k}]{$this->load[$key][$k]}<br>";
                                                        echo "T" . $tkey = array_search($key, $orginKey);
...
//                die;
    ...
    function userwise() {
        ...
        $sqlrsp = $this->ask_mysqli->select("entry", $_SESSION["db_1"]) . $this->ask_mysqli->where(array("own" => $this->winnerid, "enterydate" => date("Y-m-d"), "gametime" => $_POST["stime"], "gametimeid" => $_POST["gameid"], "gameendtime" => $_POST["etime"]), "AND");
