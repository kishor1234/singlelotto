                                                ...
                                                <div class="row">
                                                    <div class="col-lg-12">
                                                        <form action="#" method="post" id="myMainForm">
                                                            ...
                                                            <div class="form-group">
                                                                <input type="hidden" name="action" id="action" value="addAgent<?= $_SESSION["m"] ?>">
                                                                <input type="hidden" name="agent_id" id="agent_id" value="0">
                                                                <input type="hidden" name="createBy" id="createBy" value="<?= $_SESSION["userid"] ?>">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        table = $('#myTable').DataTable({
            ...
            ajax: {
                ...
                dataType: "json",
                data: {action: "loadTableAgent", m: "<?= $_SESSION["m"] ?>", createBy: "<?= $_SESSION["userid"] ?>"},
    ...
    function deleteFullAccount(id, st)
    {
        swal({
            ...
            closeOnConfirm: true
                ...
                function () {
                    $.post('<?= api_url ?>/?r=CAddSubadmin', {id: "<?= $_SESSION["userid"] ?>", agent_id: id, action: 'delete'}, function (data) {
                        console.log(data);
                        table.ajax.reload(null, false);
                        var json = JSON.parse(data);
                        $.toaster({priority: json.toast[0], title: json.toast[1], message: json.toast[2]});
    ...
    function putPoint(id, name, point)
    {
        swal({
            ...
            {
                $.post('<?= api_url ?>/?r=CAddSubadmin', {id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, subadmin: '<?= $_SESSION["userid"] ?>', action: 'putPoint'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
    ...
    function getPoint(id, name, point)
    {
        swal({
            ...
            if ($.isNumeric(inputValue))
            {
                $.post('<?= api_url ?>/?r=CAddSubadmin', {id: "<?= $_SESSION["userid"] ?>", subadmin: '<?= $_SESSION["userid"] ?>', userid: id, point: inputValue, action: 'getPoint'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
