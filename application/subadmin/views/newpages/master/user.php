...
<!--                                <form style="float:right;" id="updateBulkWinPer">
                                    <label>Change Winning %</label>
                                    <input type="hidden" id="select" name="select">
                                    <input type="number"  name="blockpers" required id="blockpers"  step="1"  min="1" max="100" maxlength="3">
                                    <input type="submit" value="Update">
                                    <input type="hidden" name="action" id="action" value="blokupdate">
                                </form>-->
...
<!--                                                            <input type="hidden" name="blockper" required id="blockper"  step="1"  min="1" max="100" value="0.00"  placeholder="Enter User Blocking Percentage" title="Enter User Blockging Percentage" autocomplete="off" class="form-control">                                                                    <label class="form-control-label">Winning Percentage(10% or 10%) </label>
                                                            <input type="hidden" name="winper" required id="winper"  step="1"  min="10" max="100" value="0.00" placeholder="Enter User Wining Percentage" title="Enter User Wining Percentage" autocomplete="off" class="form-control">-->
                                                            <!--                                                                <span id="error_name" class=""></span>
                                                                                                                        </div>-->
<!--                                                            <div class="form-group">
...
<script type="text/javascript">
    ...
    $(document).ready(function () {
        setInterval(function () {
            ...
            $("#select").val(JSON.stringify(values));

        }, 1000);
        console.log(JSON.stringify({action: "loadTable<?= $_SESSION["m"] ?>", id: "<?= $_SESSION["userid"] ?>"}));
        table = $('#myTable').DataTable({
            ...
            ajax: {
                ...
                dataType: "json",
                data: {action: "loadTable<?= $_SESSION["m"] ?>", id: "<?= $_SESSION["userid"] ?>"},
//                success: function (d) {
//                    console.log(d);
//                },
        ...
        $("#updateBulkWinPer").submit(function () {

            var formdata = new FormData($("#updateBulkWinPer")[0]);
            $.ajax({
                url: '<?= api_url ?>/?r=CAddAgentUser',
                type: 'post',
                data: formdata,
    ...
    function deleteFullAccount(id, st)
    {
        swal({
            ...
            closeOnConfirm: true
                ...
                function () {
                    $.post('<?= api_url ?>/?r=CAddAgentUser', {agent_id: "<?= $_SESSION["userid"] ?>", id: id, action: 'deleteUserDist'}, function (data) {
                        console.log(data);
                        table.ajax.reload(null, false);
                        var json = JSON.parse(data);
                        $.toaster({priority: json.toast[0], title: json.toast[1], message: json.toast[2]});
    ...
    function putPoint(id, name, point)
    {
        swal({
            ...
            {
                $.post('<?= api_url ?>/?r=CAddAgentUser', {agent_id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, action: 'putPointbysub'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
    ...
    function getPoint(id, name, point)
    {
        swal({
            ...
            if ($.isNumeric(inputValue))
            {
                $.post('<?= api_url ?>/?r=CAddAgentUser', {agent_id: "<?= $_SESSION["userid"] ?>", userid: id, point: inputValue, action: 'getPointbysub'}, function (data) {
                    console.log(data);
                    var json = JSON.parse(data);
                    if (json.status === 1)
                    {
